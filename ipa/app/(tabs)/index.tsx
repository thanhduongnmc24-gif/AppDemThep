...
export default function DemThepScreen() {
  ...
  const saveToHistory = async (original: string, processed: string | undefined, count: number) => {
    try {
      ...
      setHistory(prevHistory => {
        ...
        AsyncStorage.setItem('DEMTHEP_HISTORY', JSON.stringify(newHistory));
        return newHistory;
      });
    } catch (error) {
  ...
  const viewerResetKey = `${currentActiveItem?.id || 'empty'}-${currentMode}-${currentActiveItem?.status || 'idle'}-${displayUri ? displayUri.length : 0}`;
      ...
      <SafeAreaView style={{ flex: 1 }} edges={['top']}>
        <ScrollView ref={mainScrollRef} contentContainerStyle={styles.scrollContent}>
          ...
          <View style={styles.buttonRow}>
            ...
            <TouchableOpacity
              style={[styles.actionBtn, { backgroundColor: colors.iconBg, borderWidth: 1, borderColor: colors.border }]}
          ...
          {history.length === 0 ? (
            <Text style={{ color: colors.subText, fontStyle: 'italic', textAlign: 'center' }}>
              Chưa có lịch sử.
            </Text>
          ) : (
            history.map((item) => (
